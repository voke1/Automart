...
//  env variable is set to test for testing
...
describe('CAR', () => {
  ...
  describe('/api/v1/car/:carId/price: update price of purchase car', () => {
    ...
    it('it should return a 422 error if required fields are missing', async () => {
      const result = await chai
        .request(server)
        .patch(`/api/v1/car/${secondCarId}/price`)
        .set('token', userToken)
      ...
      result.body.should.eql({ status: 422, error: 'please fill in required fields' });
    });
